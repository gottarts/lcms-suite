...
export const CompostiTable = memo(function CompostiTable({
  ...
  const columns: Column<any>[] = useMemo(() => {
    ...
    const dataCols: Column<any>[] = [
      ...
      { key: 'data_apertura', label: 'Data Apertura', render: (v) => formatDate(v as string) },
      { key: 'concentrazione', label: 'Conc.', render: (v, row: any) => v != null && v !== '' ? `${v}${row.unita_conc ? ' ' + row.unita_conc : ''}` : '—' },
      { key: 'purezza', label: 'Purezza', render: (v) => v != null && v !== '' ? `${v}%` : '—' },
